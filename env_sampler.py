...
class TransitionBuffer(Dataset):
    def __init__(self, transition_pairs: list[tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]]):
    ...
    def sample(self, env: gym.Env, max_step=4096) -> tuple[int, int]:
        ...
        while not done:
            ...
            if action is not None:
                ...
                self.transition_pairs.append((obs, torch.tensor(action), torch.tensor(next_obs), torch.tensor(reward),))
                obs = next_obs
                reward_sum += reward
                if terminated:
                    done = True
...
class SamplerWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        self.transition_pairs.append((self.current_obs, torch.tensor(action), torch.tensor(next_obs), torch.tensor(reward),))
